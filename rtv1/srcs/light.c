...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/01/30 12:58:49 by oseitama          #+#    #+#             */
/*   Updated: 2023/01/30 14:41:01 by oseitama         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	light_data(t_sdl *sdl, char **str)
{
	...
	i = -1;
	sdl->light[sdl->light_counter].pos.x = (double)(ft_atoi(str[1]));
	sdl->light[sdl->light_counter].pos.y = (double)(ft_atoi(str[2]));
	sdl->light[sdl->light_counter].pos.z = (double)(ft_atoi(str[3]));
	sdl->light[sdl->light_counter].inten = (double)(ft_atoi(str[4]) / 100);
	if (sdl->light[sdl->light_counter].inten < 0)
		ft_error("Wrong intensity paramenter");
	sdl->light_counter++;
	while (++i <= 4)
		free(str[i]);
	free(str);
...
int	shadow_init(t_light *light, t_sdl *sdl)
{
	...
	light->p = vec_sub(light->p, vec_scale(dir, EPS));
	while (++i < sdl->obj_num)
	{
...
void	get_intensity(t_sdl *sdl, t_light *light, t_vec v, double s)
{
	...
	if (shadow_init(light, sdl) == 0)
		inten = light->inten;
	if (n_dot_l > EPS)
		light->new_inten += inten * (n_dot_l / \
			(vec_len(light->n) * vec_len(l)));
	if (s > 0)
	{
		...
		r_dot_v = vec_dot(r, v);
		if (r_dot_v > EPS)
			light->new_inten += inten * ft_pow((r_dot_v / (vec_len(r) * \
				vec_len(v))), s);
	}
}
